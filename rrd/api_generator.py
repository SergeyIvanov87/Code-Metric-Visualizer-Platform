...
def make_script_rrd_collect(script, desired_file_ext=""):
    ...
    body = (
        ...
        *api_fs_exec_utils.generate_read_api_fs_args(), r"",
        ...
        r'if [ ! -z ${SESSION_ID_VALUE} ]; then echo "SESSION_ID=${SESSION_ID_VALUE} ${IN_ARGS[@]}" > ${SHARED_API_DIR}/${MAIN_SERVICE_NAME}/cc/analytic/PUT/exec; fi',
        r'if [ -z ${SESSION_ID_VALUE} ]; then echo "SESSION_ID=`hostname` ${IN_ARGS[@]}" > ${SHARED_API_DIR}/${MAIN_SERVICE_NAME}/cc/analytic/PUT/exec; fi',
        ...
        r'if [ ! -z ${SESSION_ID_VALUE} ]; then echo "SESSION_ID=`hostname`_${SESSION_ID_VALUE} ${IN_ARGS[@]} $(replace_space_in_even_position "${ONLY_METRICS_IN_RANGE}")" > ${SHARED_API_DIR}/${MAIN_SERVICE_NAME}/cc/statistic/GET/exec; fi',
        r'if [ -z ${SESSION_ID_VALUE} ]; then echo "SESSION_ID=`hostname` ${IN_ARGS[@]} $(replace_space_in_even_position "${ONLY_METRICS_IN_RANGE}")" > ${SHARED_API_DIR}/${MAIN_SERVICE_NAME}/cc/statistic/GET/exec; fi',
