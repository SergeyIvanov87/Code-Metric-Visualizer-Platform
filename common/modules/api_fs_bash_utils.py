...
def __split_quoted_arguments_function__(override_function_name):
    ...
    return function_name, [
        ...
        '    output_ref=()\n',
        '    # Consume ordinary text in chunks. Taking a one-character Bash\n',
        '    # substring for every byte makes long arguments quadratic.\n',
        '    while [[ -n "${input}" ]]; do\n',
        r'        if ((!escaped)) && [[ "${input}" =~ ^([^\\\"[:space:]]+) ]]; then' + '\n',
        ...
        '        input="${input:1}"\n',
        '        if ((escaped)); then\n',
        '            if ((in_quotes)) && [[ "${character}" != \'"\' && "${character}" != \'\\\' ]]; then\n',
        "                current+='\\'\n",
        '            fi\n',
        '            current+="${character}"\n',
        '            escaped=0\n',
        ...
        '        case "${character}" in\n',
        '            \\\\)\n',
        '                # Defer handling until the next character so ordinary\n',
        '                # backslashes inside quotes can remain literal.\n',
        '                escaped=1\n',
        '                argument_started=1\n',
        '                ;;\n',
        '            \'"\')\n',
...
def __extract_attr_value_from_string_using_tokenizer_function__():
    ...
    return function_name, [
        *generate_split_quoted_arguments("split_quoted_arguments_inner"), r"",
        ...
        '    local value=""\n',
        '    local arg\n',
        '    local safe_input_pattern\n',
        '    local safe_value_pattern\n',
        '    local -a args\n',
        '    if [[ -z "${delimiter}" ]]; then\n',
        '       printf "Delimiter cannot be empty\\n" >&2\n',
        '       return 1\n',
        '    fi\n',
        '    # Base64 and similarly simple quoted values are common filesystem-API\n',
        '    # payloads. Extract them without repeatedly tokenizing a potentially\n',
        '    # large input for every attribute requested by an executor.\n',
        '    safe_input_pattern=\'^([^"\\\\]|\\\\.|"([^"\\\\]|\\\\.)*")*$\'\n',
        '    safe_value_pattern="(^|[[:space:]])${attr}${delimiter}\\\"([[:alnum:]+/=]*)\\\"($|[[:space:]])"\n',
        '    if [[ "${str}" =~ ${safe_input_pattern} ]] && [[ "${str}" =~ ${safe_value_pattern} ]]; then\n',
